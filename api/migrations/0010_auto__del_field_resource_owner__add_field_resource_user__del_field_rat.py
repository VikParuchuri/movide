...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Rating', fields ['message', 'owner']
        db.delete_unique(u'api_rating', ['message_id', 'owner_id'])

        # Deleting field 'Resource.owner'
        db.delete_column(u'api_resource', 'owner_id')

        # Adding field 'Resource.user'
        db.add_column(u'api_resource', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='resources', to=orm['auth.User']),
                      keep_default=False)

        # Deleting field 'Rating.owner'
        db.delete_column(u'api_rating', 'owner_id')

        # Adding field 'Rating.user'
        db.add_column(u'api_rating', 'user',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='ratings', null=True, on_delete=models.SET_NULL, to=orm['auth.User']),
                      keep_default=False)

        # Adding unique constraint on 'Rating', fields ['message', 'user']
        db.create_unique(u'api_rating', ['message_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Rating', fields ['message', 'user']
        db.delete_unique(u'api_rating', ['message_id', 'user_id'])

        # Adding field 'Resource.owner'
        db.add_column(u'api_resource', 'owner',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, related_name='resources', to=orm['auth.User']),
                      keep_default=False)

        # Deleting field 'Resource.user'
        db.delete_column(u'api_resource', 'user_id')

        # Adding field 'Rating.owner'
        db.add_column(u'api_rating', 'owner',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='ratings', null=True, to=orm['auth.User'], on_delete=models.SET_NULL, blank=True),
                      keep_default=False)

        # Deleting field 'Rating.user'
        db.delete_column(u'api_rating', 'user_id')

        # Adding unique constraint on 'Rating', fields ['message', 'owner']
        db.create_unique(u'api_rating', ['message_id', 'owner_id'])
    ...
    models = {
        ...
        u'api.resource': {
            'Meta': {'unique_together': "(('classgroup', 'name'),)", 'object_name': 'Resource'},
            'approved': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
